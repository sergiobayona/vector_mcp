...
server.enable_authentication!(
  strategy: :api_key,
  keys: ['admin-secret-key', 'user-read-key', 'demo-key-123']
)
...
server.enable_authorization! do
  ...
  authorize_tools do |user, action, tool|
    ...
    when 'secure_data'
      # Only authenticated users can access secure data
      user.present?
    when 'admin_tool'
      # Only admin API key can use admin tools
      user&.dig(:api_key) == 'admin-secret-key'
    else
      # Default: require authentication for all other tools
      user.present?
    end
  end
end
...
puts "\n🚀 Starting server..."
puts "\nUsage Examples:"
puts "  Public tool (no auth): echo '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/call\",\"params\":{\"name\":\"public_info\",\"arguments\":{\"topic\":\"weather\"}}}'"
