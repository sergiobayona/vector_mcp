...
class DataAnalysisServer
  ...
  def register_data_tools
    ...
    @server.register_tool(
      ...
      unless dataset
        return { success: false, error: "Dataset not found: #{dataset_name}" }
      end
  ...
  def register_analysis_tools
    ...
    @server.register_tool(
      ...
      input_schema: {
        ...
        required: ["dataset_name", "time_column", "value_column"],
        additionalProperties: false
      }
    ) do |arguments, session_context|
      dataset_name = arguments["dataset_name"]
  ...
  def register_export_tools
    ...
    @server.register_tool(
      ...
      input_schema: {
        ...
        required: ["results", "format", "filename"],
        additionalProperties: false
      }
    ) do |arguments, session_context|
  ...
  def load_api_data(source, headers, options)
  ...
  def assess_data_quality(data)
    ...
    columns.each do |column|
      ...
      if column == columns.first
        ...
        issues << "#{duplicate_count} duplicate rows found" if duplicate_count > 0
      end
    end
  ...
  def parse_time(value)
    Time.parse(value.to_s) rescue Time.at(0)
  end
  ...
  def calculate_trend_strength(moving_avg)
    ...
    consistent_direction = changes.count { |c| c.positive? } - changes.count { |c| c.negative? }
    (consistent_direction.abs.to_f / changes.size).round(2)
  end

  def calculate_quality_score(data, issues)
    base_score = 100
  ...
  def generate_html_report(data, metadata, include_charts)
