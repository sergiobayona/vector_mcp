...
class WebScrapingServer
    ...
    @server.register_tool(
      ...
      begin
        ...
        results = scrape_multiple_pages_worker(
          base_url, link_pattern, max_pages, delay, extract_type, follow_depth, job_id
        )
  ...
  def register_processing_tools
    ...
    @server.register_tool(
      ...
      input_schema: {
        ...
        required: ["content_id", "operations"],
        additionalProperties: false
      }
    ) do |arguments, session_context|
      ...
      unless scraped_content
        return { success: false, error: "Content not found: #{content_id}" }
      end
    ...
    @server.register_tool(
      ...
      begin
        ...
        if filter["extract_type"]
          filtered_data = filtered_data.select { |_, data| data[:extract_type] == filter["extract_type"] }
        end
  ...
  def extract_full_content(html, remove_ads, remove_navigation)
    ...
    if remove_navigation
      content = remove_navigation_content(content)
    end
  ...
  def extract_main_content(html, remove_ads, remove_navigation)
    ...
    main_content_patterns = [
      /<article[^>]*>(.*?)<\/article>/mi,
      /<main[^>]*>(.*?)<\/main>/mi,
      /<div[^>]*class="[^"]*content[^"]*"[^>]*>(.*?)<\/div>/mi,
      /<div[^>]*id="[^"]*content[^"]*"[^>]*>(.*?)<\/div>/mi
    ]
  ...
  def extract_links(html, base_url)
    ...
    html.scan(/<a[^>]*href="([^"]*)"[^>]*>(.*?)<\/a>/mi) do |href, text|
  ...
  def scrape_multiple_pages_worker(base_url, link_pattern, max_pages, delay, extract_type, follow_depth, job_id)
    results = []
  ...
  def remove_navigation_content(html)
    nav_selectors = [
      /<nav[^>]*>.*?<\/nav>/mi,
      /<div[^>]*class="[^"]*nav[^"]*"[^>]*>.*?<\/div>/mi,
      /<div[^>]*class="[^"]*menu[^"]*"[^>]*>.*?<\/div>/mi,
      /<header[^>]*>.*?<\/header>/mi,
      /<footer[^>]*>.*?<\/footer>/mi
    ]
  ...
  def extract_html_tables(html, has_headers)
    ...
    html.scan(/<table[^>]*>(.*?)<\/table>/mi) do |table_content|
      ...
      table_content[0].scan(/<tr[^>]*>(.*?)<\/tr>/mi) do |row_content|
        cells = []
        row_content[0].scan(/<t[hd][^>]*>(.*?)<\/t[hd]>/mi) do |cell_content|
  ...
  def extract_html_forms(html)
    ...
    html.scan(/<form[^>]*>(.*?)<\/form>/mi) do |form_content|
      form_data = {
  ...
  def extract_form_fields(form_html)
    ...
    form_html.scan(/<select[^>]*name="([^"]*)"[^>]*>(.*?)<\/select>/mi) do |name, options|
      fields << {
        type: "select",
        name: name,
  ...
  def extract_select_options(options_html)
    ...
    options_html.scan(/<option[^>]*value="([^"]*)"[^>]*>(.*?)<\/option>/mi) do |value, text|
      options << {
        value: value,
        text: clean_html_content(text)
      }
  ...
  def extract_keywords(text, count)
    ...
    stop_words = %w[the and or but in on at to for of with by from a an is are was were be been being have has had will would could should may might must can]
    ...
    CSV.generate do |csv|
      ...
      data.each do |content_id, content_data|
        ...
        unless include_metadata
          content_text = content_data[:content].is_a?(Hash) ? 
  ...
  def save_export(data, filename, format)
  ...
  def generate_markdown_report(include_samples)
    ...
    @scraped_data.each do |content_id, data|
      md << "### #{data[:url]}"
      md << ""
  ...
  def generate_text_report(include_samples)
    lines = []
  ...
  def extract_ratings(content)
    content.scan(/(\d+(?:\.\d+)?)\s*(?:stars?|\/5|rating)/i).flatten
  end

  def extract_product_descriptions(content)
    # Extract paragraphs that might be descriptions
    content.scan(/<p[^>]*>(.*?)<\/p>/mi).map { |match| clean_html_content(match[0]) }.select { |text| text.length > 50 }
  end
  ...
  def extract_selected_content(content, selector)
