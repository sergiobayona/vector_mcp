...
class WebScrapingServer
    ...
    @server.register_tool(
      ...
      input_schema: {
        ...
        properties: {
          ...
          max_length: { type: "integer", description: "Maximum content length in characters", default: 10_000 },
      ...
      max_length = args.fetch("max_length", 10_000)
      ...
      begin
        ...
        text_content = "#{text_content[0, max_length]}... (truncated)" if text_content.length > max_length
    ...
    @server.register_tool(
      ...
      input_schema: {
        ...
        properties: {
          ...
          pattern_type: {
            type: "string",
            enum: %w[email phone price url custom],
        ...
        required: %w[job_id pattern_type]
      }
    ) do |args, _session|
      job_id = args["job_id"]
    ...
    @server.register_tool(
      ...
      input_schema: {
        ...
        properties: {
          ...
          data_type: {
            type: "string",
            enum: %w[table list headings],
            description: "Type of structured data to extract"
          }
        },
        required: %w[job_id data_type]
      }
    ) do |args, _session|
      job_id = args["job_id"]
    ...
    @server.register_tool(
      ...
      input_schema: {
        ...
        properties: {
          ...
          format: { type: "string", enum: %w[json text], description: "Output format", default: "json" }
        },
        required: %w[job_id filename]
      }
    ) do |args, _session|
      job_id = args["job_id"]
    ...
    @server.register_tool(
      ...
      report = {
        ...
        jobs: @scraped_data.map do |job_id, data|
          ...
          job_summary[:content] = data[:content] if include_content
  ...
  def extract_links(html, base_uri, filter_domain: false)
    ...
    html.scan(/href=['"]([^'"]+)['"]/i) do |match|
      ...
      begin
        ...
        next if filter_domain && link_uri.host != base_uri.host
  ...
  def extract_tables(html)
    ...
    html.scan(%r{<table[^>]*>(.*?)</table>}mi) do |table_content|
      rows = table_content[0].scan(%r{<tr[^>]*>(.*?)</tr>}mi).map do |row_content|
        row_content[0].scan(%r{<t[hd][^>]*>(.*?)</t[hd]>}mi).map do |cell_content|
  ...
  def extract_lists(html)
    ...
    html.scan(%r{<[ou]l[^>]*>(.*?)</[ou]l>}mi) do |list_content|
      items = list_content[0].scan(%r{<li[^>]*>(.*?)</li>}mi).map do |item_content|
  ...
  def extract_headings(html)
    ...
    (1..6).each do |level|
      html.scan(%r{<h#{level}[^>]*>(.*?)</h#{level}>}mi) do |heading_content|
