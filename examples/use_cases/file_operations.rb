...
class FileOperationsServer
          ...
          begin
            ...
            if search_content.include?(search_pattern)
              ...
              lines.each_with_index do |line, index|
                ...
                if search_line.include?(search_pattern)
                  matches << {
                    line_number: index + 1,
                    line: line.strip,
  ...
  def text_file?(path)
    ...
    sample = File.read(path, 1024) rescue ""
  ...
  def detect_file_type(content, extension)
    ...
    return "ruby" if extension == ".rb" || content.match?(/^#!/usr/bin/env ruby/)
    return "python" if extension == ".py" || content.match?(/^#!/usr/bin/env python/)
    return "javascript" if extension == ".js" || content.match?(/^#!/usr/bin/env node/)
  ...
  def generate_directory_report(path, include_analysis)
    ...
    report = {
      ...
      summary: {
        ...
        largest_files: files.sort_by { |f| -File.size(f) }.first(5).map { |f| 
