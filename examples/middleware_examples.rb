...
def demo_middleware
  ...
  server.use_middleware(MiddlewareExamples::CustomLoggingMiddleware, %i[
                          before_tool_call after_tool_call on_tool_error
                        ], priority: 10, conditions: { include_params: true })

  server.use_middleware(MiddlewareExamples::RateLimitingMiddleware, :before_tool_call,
                        priority: 20,
                        conditions: { limits: { echo: 5, default: 10 } })

  server.use_middleware(MiddlewareExamples::PiiRedactionMiddleware, %i[
                          before_tool_call after_tool_call
                        ], priority: 30)

  server.use_middleware(MiddlewareExamples::RetryMiddleware, :on_tool_error,
                        priority: 40,
