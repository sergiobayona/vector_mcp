...
module VectorMCP
  module Browser
    ...
    module Tools
      ...
      class Base
        ...
        def sanitize_params_for_logging(params)
          ...
          sanitized.each do |key, value|
            sanitized[key] = "#{value[0...500]}...[TRUNCATED]" if value.is_a?(String) && value.length > 500
          end
      ...
      # Snapshot tool
      class Snapshot < Base
        def call(_arguments, _session_context = nil)
          result = make_browser_request("snapshot", {})

          raise OperationError, result["error"] unless result["success"]
      ...
      class Screenshot < Base
        def call(_arguments, _session_context = nil)
          ...
          base64_data = if screenshot_data.start_with?("data:image/")
                          screenshot_data.split(",", 2)[1]
                        else
                          screenshot_data
                        end
      ...
      # Wait tool
      class Wait < Base
        def call(arguments, _session_context = nil)
