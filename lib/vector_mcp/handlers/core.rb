  ...
  module Handlers
    ...
    module Core
      ...
      def self.call_tool(params, session, server)
        ...
        result = tool.handler.call(arguments, security_result[:session_context])
        {
          isError: false,
      ...
      def self.read_resource(params, session, server)
        ...
        # Let StandardError propagate to Server#handle_request
        content_raw = resource.handler.call(params, security_result[:session_context])
      ...
      def self.handle_security_failure(security_result)
        ...
        when 'AUTHENTICATION_REQUIRED'
          raise VectorMCP::UnauthorizedError.new(security_result[:error])
        when 'AUTHORIZATION_FAILED'
          raise VectorMCP::ForbiddenError.new(security_result[:error])
        else
          # Fallback to generic unauthorized error
          raise VectorMCP::UnauthorizedError.new(security_result[:error] || 'Security check failed')
        end
      end
