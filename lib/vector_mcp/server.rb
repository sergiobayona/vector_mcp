...
module VectorMCP
  ...
  class Server
    ...
    # @return [void]
    def enable_authentication!(strategy: :api_key, **options)
      ...
      when :custom
        add_custom_auth(&options[:handler])
      else
    ...
    # @return [void]
    def add_custom_auth(&handler)
      strategy = Security::Strategies::Custom.new(&handler)
